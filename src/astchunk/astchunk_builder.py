...
class ASTChunkBuilder:
    ...
    def _prebind_decorators(self, nodes: list) -> list:
        ...
        while i < len(nodes):
            ...
            if node_type in self.DECORATOR_TYPES:
                ...
                while (
                    i < len(nodes)
                    and getattr(nodes[i], "type", "") in self.DECORATOR_TYPES
                ):
                ...
                if (
                    i < len(nodes)
                    and getattr(nodes[i], "type", "") in self.DEFINITION_TYPES
                ):
                    group.append(nodes[i])
                    i += 1
            ...
            # Check if this is a comment immediately before a definition
                ...
                node_type == "comment"
                and i + 1 < len(nodes)
                and getattr(nodes[i + 1], "type", "") in self.DEFINITION_TYPES
            ):
    ...
    def assign_nodes_to_windows(
        ...
        _prebind_active: bool | None = None,
    ) -> Generator[list[ASTNode], None, None]:
        """
        ...
        for node in nodes:
            ...
            if (len(current_window) == 0 and node_exceeds_limit) or (
                ...
                if node_exceeds_limit:
                    ...
                    childs_ancestors = (
                        ancestors if is_node_group else ancestors.append(node)
                    )
